<template>
  <section
    class="w-full py-20 space-y-14 relative border-t border-white border-opacity-20 overflow-hidden"
  >
    <div>
      <img
        src="/images/fifthsection/topeffect.png"
        class="w-full h-full object-cover absolute -top-[65rem] sm:-top-[45rem] lg:-top-[26rem] left-0"
        alt=""
      />
    </div>
    <div
      class="flex flex-col items-center gap-6 px-4 text-center z-10 relative"
    >
      <span
        class="font-satoshi font-black text-sm text-white text-opacity-60 uppercase"
        >who do we hire</span
      >

      <h3 class="text-white font-satoshi font-bold text-4xl">
        Lima Protection is for the elite
      </h3>
      <p class="font-satoshi font-normal text-base text-white text-opacity-60">
        We prospect qualified candidates to be part of our elite protection team
      </p>

      <button class="element rounded-full p-px group duration-300">
        <div
          class="rounded-full py-[12px] px-[20px] text-white font-satoshi font-medium bg-black duration-300 bg-gradient-to-br group-hover:from-[#ffffff80] group-hover:to-[#00000001] from-[#ffffff30] from-[9.11%] to-[#00000001] to-[76.61%]"
        >
          Apply for program
        </div>
      </button>
    </div>

    <div
      class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-[1280px] mx-auto px-4 xl:px-0"
    >
      <div class="w-full h-[340px] rounded-[12px] relative overflow-hidden">
        <img
          src="/images/fifthsection/FormerPolicemen.png"
          class="w-full h-full object-cover"
          alt=""
        />
        <div class="absolute bottom-8 left-8">
          <span class="text-white font-satoshi font-bold text-[28px]"
            >Former Policemen</span
          >
        </div>
      </div>
      <div class="w-full h-[340px] rounded-[12px] relative overflow-hidden">
        <img
          src="/images/fifthsection/MilitaryPersonnel.png"
          class="w-full h-full object-cover"
          alt=""
        />
        <div class="absolute bottom-8 left-8">
          <span class="text-white font-satoshi font-bold text-[28px]"
            >Military Personnel</span
          >
        </div>
      </div>
      <div class="w-full h-[340px] rounded-[12px] relative overflow-hidden">
        <img
          src="/images/fifthsection/LawEnforcement.png"
          class="w-full h-full object-cover"
          alt=""
        />
        <div class="absolute bottom-8 left-8">
          <span class="text-white font-satoshi font-bold text-[28px]"
            >Law Enforcement</span
          >
        </div>
      </div>
    </div>
  </section>
</template>

<script setup>
import ClientsCard from "@/components/UI/ClientsCard.vue";
import ServicesCard from "@/components/UI/ServicesCard.vue";
import PortfolioCard from "@/components/UI/PortfolioCard.vue";
</script>
